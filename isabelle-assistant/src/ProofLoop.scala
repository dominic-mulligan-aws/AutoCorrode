...
object ProofLoop {
  ...
  private def fillPhase(
    ...
    Output.writeln(s"[Assistant] fillPhase initial sketch (${proof.length} chars):\n---\n$proof\n---")
    var step = 0
    var refinementCount = 0  // Track total refinements to prevent runaway

    while (countSorries(proof) > 0 && step < maxFillAttempts && !AssistantDockable.isCancelled && System.currentTimeMillis() < deadline) {
      val remaining = countSorries(proof)
      step += 1
      AssistantDockable.setStatus(s"Proving... step $step ($remaining sorries remaining)")
      ...
      val filled = filledSimple.orElse {
        ...
        if (!AssistantDockable.isCancelled && System.currentTimeMillis() < deadline && failedErrors.nonEmpty) {
  ...
  private def speculateLemma(
    ...
    try {
      ...
      Thread.sleep(AssistantConstants.PIDE_PROCESSING_DELAY)
      ...
      case ex: Exception =>
        Output.writeln(s"[Assistant] Lemma speculation failed: ${ex.getMessage}")
        None
    }
  }
  ...
  private def proveLemmaFull(
    ...
    val deadline = System.currentTimeMillis() + AssistantConstants.PROVE_LEMMA_SUB_DEADLINE
    ...
    if (AssistantDockable.isCancelled || System.currentTimeMillis() > deadline) return None
  ...
  private def fillPhaseLimited(
    ...
    while (countSorries(proof) > 0 && step < maxSteps && !AssistantDockable.isCancelled && System.currentTimeMillis() < deadline) {
      val remaining = countSorries(proof)
      step += 1
      ...
      case Some((prefix, _)) if prefix.trim.nonEmpty =>
        ...
        stepResult match {
          case Right(result) if result.stateText.trim.nonEmpty
            ...
            Output.writeln(s"[Assistant] Got actual subgoal from prefix: ${result.stateText.take(200)}")
            chat(s"Subgoal:\n```\n${result.stateText.linesIterator.take(8).mkString("\n")}\n```")
            result.stateText
          case Right(result) =>
            Output.writeln(s"[Assistant] Prefix returned complete/empty state, falling back")
            initialGoalState(view)
          case Left(err) =>
            Output.writeln(s"[Assistant] Prefix execution failed ($err), falling back")
            initialGoalState(view)
        }
      case _ =>
        Output.writeln("[Assistant] No prefix before sorry, using original goal")
        initialGoalState(view)
    }
  }
  ...
  private def fillViaLLM(
    ...
    if (candidates.isEmpty) {
      ...
      for (fill <- candidates if result.isEmpty && !AssistantDockable.isCancelled) {
  ...
  def run(view: View, command: Command, commandText: String, initialGoalState: String): Unit = {
    ...
    chat(s"**Auto-prove** starting (branches=$numBranches, max fills=$maxFillAttempts, timeout=${AssistantOptions.getProveTimeout / 1000}s)")

    // Get structured goal analysis from PIDE markup
    val goalAnalysis = analyzeGoalAtCursor(view)
    val freeVars = goalAnalysis.map(_.freeVars).getOrElse(Nil)
    if (freeVars.nonEmpty) Output.writeln(s"[Assistant] Free variables from PIDE: ${freeVars.mkString(", ")}")
    ...
    if (skeleton.isEmpty || AssistantDockable.isCancelled || System.currentTimeMillis() > deadline) {
    ...
    GUI_Thread.later {
      ...
      proof match {
        case Some(code) if totalRemaining == 0 =>
          AssistantDockable.respondInChat(s"[ok] **Proof found** ($reason):",
            Some((code, InsertHelper.createInsertAction(view, code))))
          AssistantDockable.setBadge(VerificationBadge.Verified(None))
        case Some(code) =>
          AssistantDockable.respondInChat(s"**Partial proof** ($reason):",
            Some((code, InsertHelper.createInsertAction(view, code))))
