...
object ErrorHandler {
  ...
  def sanitize(input: String, maxLength: Int = AssistantConstants.MAX_RESPONSE_LENGTH): String =
    if (input == null) ""
  ...
  def logOperation[T](operation: String)(block: => T): T = {
    ...
    try {
      ...
      case ex: Exception =>
        ...
        safeErrorMessage(s"[Assistant] $operation failed after ${elapsed}ms: $userMessage")
