...
object ProofPlanner {
  ...
  def createProofPlan(commandText: String, goalState: String, context: String): String = {
    ...
    try {
      ...
      planResult match {
        case Right(plan) =>
          Output.writeln(s"[Assistant] Proof plan:\n$plan")
          plan
        case Left(ex) =>
          Output.writeln(s"[Assistant] Proof planning failed: ${ex.getMessage}")
  ...
  def fetchRichContext(
    ...
    val timeout = AssistantConstants.CONTEXT_FETCH_TIMEOUT
    ...
    if (constants.nonEmpty && IQAvailable.isAvailable) {
      ...
      Output.writeln(s"[Assistant] Searching theorems for: $criteria")
      GUI_Thread.later {
        IQIntegration.runFindTheoremsAsync(view, command, criteria,
          AssistantOptions.getFindTheoremsLimit, AssistantOptions.getFindTheoremsTimeout, {
    ...
    val waitMs = timeout + AssistantConstants.TIMEOUT_BUFFER_MS
  ...
  private def fetchDefinitionsViaIQ(
    ...
    try {
      ...
      case ex: Exception =>
        Output.writeln(s"[Assistant] fetchDefinitionsViaIQ failed: ${ex.getMessage}")
        ""
    }
  }
}