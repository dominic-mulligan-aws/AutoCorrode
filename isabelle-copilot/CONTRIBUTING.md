# Contributing to Isabelle Copilot

## Architecture Overview

Isabelle Copilot is a jEdit plugin written in Scala 3 that integrates AWS Bedrock LLMs with the Isabelle proof assistant. See the [Architecture section in README.md](README.md#architecture) for the high-level component map.

### Key Components

| Component | Purpose |
|-----------|---------|
| `CopilotPlugin` | jEdit plugin lifecycle (start/stop) |
| `CopilotDockable` | Chat UI panel (Swing, singleton) |
...
- All `CopilotDockable` UI updates go through `GUI_Thread.later`.
...
1. Add a `ToolDef` to the `tools` list in `CopilotTools.scala`.
2. Add a `case "tool_name" =>` handler in `CopilotTools.executeTool`.
...
- Log operations with `Output.writeln(s"[Copilot] ...")`