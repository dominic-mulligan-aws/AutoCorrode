 ...
 * and other Isabelle query capabilities. When unavailable, Copilot runs in
 * LLM-only mode with reduced functionality.
 *
 * The availability status is cached but can be refreshed via `recheck()`,
 * for example if I/Q is installed after Copilot starts.
 */
object IQAvailable {
  ...
  def recheck(): Boolean = {
    ...
    if (changed) {
      logStatus()
      CopilotDockable.refreshIQStatus()
    }
    result
  }
  ...
  def logStatus(): Unit = {
    if (isAvailable)
      Output.writeln("[Copilot] I/Q plugin detected - proof verification features enabled")
    else
      Output.writeln("[Copilot] I/Q plugin not found - running in LLM-only mode. Restart Isabelle/jEdit after installing I/Q to enable verification.")
  }
}