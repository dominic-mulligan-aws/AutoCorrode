...
object FindTheoremsAction {
  def findTheorems(view: View, initialPattern: Option[String]): Unit = {
    ...
    if (patternOpt.isEmpty) ()
      ...
      GUI.warning_dialog(view, "Isabelle Copilot", "I/Q plugin not available")
    } else {
      ...
      if (commandOpt.isEmpty) {
        GUI.warning_dialog(view, "Isabelle Copilot", "No Isabelle command at cursor position")
      } else {
        ...
        IQIntegration.runFindTheoremsAsync(
          ...
          CopilotOptions.getFindTheoremsLimit,
          CopilotOptions.getFindTheoremsTimeout,
          {
            case Right(results) => GUI_Thread.later { displayResults(view, pattern, results) }
            case Left(error) => GUI_Thread.later {
              CopilotDockable.respondInChat(s"Find theorems error: $error")
  ...
  private def displayResults(view: View, pattern: String, results: List[String]): Unit = {
    ...
    if (results.isEmpty) {
      CopilotDockable.respondInChat(s"No theorems found matching: $pattern")
    } else {
