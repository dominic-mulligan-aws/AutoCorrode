...
object CopilotTools {
  ...
  def executeTool(name: String, args: Map[String, Any], view: View): String = {
    Output.writeln(s"[Copilot] Tool call: $name(${args.map { case (k, v) => s"$k=${v.toString.take(100)}" }.mkString(", ")})")
    CopilotDockable.setStatus(s"[tool] $name...")
    try {
      name match {
  ...
  private def execSearchInTheory(args: Map[String, Any], view: View): String = {
    ...
    safeTheoryArg(args) match {
      ...
      findBuffer(normalized) match {
        ...
        case Some(buffer) =>
          val max = math.min(CopilotConstants.MAX_SEARCH_RESULTS, math.max(1, intArg(args, "max_results", 20)))
  ...
  private def execFindTheorems(args: Map[String, Any], view: View): String = {
    ...
    if (pattern.isEmpty) "Error: pattern required"
      ...
      cmdLatch.await(CopilotConstants.GUI_DISPATCH_TIMEOUT_SEC, TimeUnit.SECONDS)

      commandOpt match {
        case None => "No Isabelle command at cursor position."
        case Some(command) =>
          val max = math.min(CopilotConstants.MAX_FIND_THEOREMS_RESULTS, math.max(1, intArg(args, "max_results", 20)))
