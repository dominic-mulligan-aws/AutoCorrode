...
object IQExecutor {
  ...
  def executeStep(
    ...
    val timeoutMs = CopilotOptions.getProveStepTimeout
    ...
    GUI_Thread.later {
      IQIntegration.executeStepAsync(view, command, proofText, timeoutMs, { r =>
        Output.writeln(s"[Copilot] executeStep result: $r")
        result = r
        latch.countDown()
      })
    }
  ...
  def analyzeGoalAtCursor(view: View): Option[GoalExtractor.GoalAnalysis] = {
    GUIThreadUtil.awaitOnGUIThread(CopilotConstants.GUI_DISPATCH_TIMEOUT) {
      ...
      for (method <- methods) {
        ...
        GUI_Thread.later {
          IQIntegration.verifyProofAsync(
            view, command, method, 
            CopilotOptions.getProveStepTimeout, 
            { r =>
              result = Some(r)
              latch.countDown()
            }
        ...
        latch.await(CopilotOptions.getProveStepTimeout + 2000, TimeUnit.MILLISECONDS)
