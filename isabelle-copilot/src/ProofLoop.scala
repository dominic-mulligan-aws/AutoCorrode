 ...
 */
object ProofLoop {
  private def maxRetries: Int = CopilotOptions.getProveRetriesPerStep
  private def stepTimeoutMs: Long = CopilotOptions.getProveStepTimeout
  private def numBranches: Int = CopilotOptions.getProveBranches
  private def maxFillAttempts: Int = CopilotOptions.getProveMaxSteps
  ...
  private def extractCode(response: String): String = ProofTextUtil.extractCode(response)
  private def countSorries(proof: String): Int = ProofTextUtil.countSorries(proof)
  private def countUnfilled(proof: String): Int = ProofTextUtil.countUnfilled(proof)
  private def splitAtSorry(proof: String, n: Int): Option[(String, String)] = ProofTextUtil.splitAtSorry(proof, n)
  private def replaceSorry(proof: String, n: Int, replacement: String): Option[String] = ProofTextUtil.replaceSorry(proof, n, replacement)
  private def markSorry(proof: String, n: Int): String = ProofTextUtil.markSorry(proof, n)

  // --- Context fetching ---

  // Delegate to ProofPlanner module
  private def fetchRichContext(
    view: View, command: Command, goalState: String,
  ...
  private def executeStep(view: View, command: Command, proofText: String): Either[String, IQIntegration.ProofStepResult] = {
    Output.writeln(s"[Copilot] executeStep sending:\n$proofText")
    val result = IQExecutor.executeStep(view, command, proofText)
    Output.writeln(s"[Copilot] executeStep result: $result")
    result
  }

  private def initialGoalState(view: View): String = IQExecutor.getGoalState(view)
  private def analyzeGoalAtCursor(view: View): Option[GoalExtractor.GoalAnalysis] = IQExecutor.analyzeGoalAtCursor(view)
  ...
  private def isStructurallyComplete(proof: String): Boolean = ProofTextUtil.isStructurallyComplete(proof)
  private val unfilledPattern = ProofTextUtil.unfilledPattern
  private val sorryPattern = ProofTextUtil.sorryPattern

  // --- LLM interaction ---

  private val llmCallTimeoutMs = CopilotConstants.PROVE_LLM_CALL_TIMEOUT

  /** 
   * Invoke LLM with a timeout guard. Throws on timeout or LLM error.
   * Uses invokeNoCache to avoid polluting chat history with auto-prover prompts.
   */
  private def invokeLLMWithTimeout(prompt: String): String = {
    val latch = new CountDownLatch(1)
    @volatile var result: Either[Exception, String] = Left(new RuntimeException("LLM call timed out"))
  ...
  private val simpleMethods: List[String] = List(
    "by simp", "by auto", "by blast", "by force", "by fastforce",
    "by simp_all", "by argo", "by arith", "by linarith", "by presburger",
    "by meson", "by metis", "by smt"
  )

  /** Build induction/cases candidates from actual free variables (from PIDE markup). */
  ...
  private def tryMethodsViaIQ(
    view: View, command: Command, methods: List[String], label: String
  ): Option[String] = {
    ...
    boundary {
      for (method <- methods) {
        if (CopilotDockable.isCancelled) break(None)
        val latch = new CountDownLatch(1)
        @volatile var result: Option[IQIntegration.VerificationResult] = None
        GUI_Thread.later {
          IQIntegration.verifyProofAsync(view, command, method, stepTimeoutMs, { r =>
            result = Some(r)
            latch.countDown()
          })
        }
  ...
  /** Try simple methods as actual replacements in the proof skeleton.
   *  Each method is substituted for the first sorry and verified via I/Q.
   *  Returns Some(newProof) with the sorry replaced, or None. */
  private def trySimpleReplacements(
    view: View, command: Command, proof: String
  ): Option[String] = {
    import scala.util.boundary, boundary.break
   ...
   * Extracts the text that replaced the first sorry by diffing prefix/suffix.
   */
  ...
  // Delegate to ProofPlanner module
  private def createProofPlan(commandText: String, goalState: String, context: String): String =
    ProofPlanner.createProofPlan(commandText, goalState, context)

  // --- Phase 0: Simple methods ---

  /** Try simple one-liner proofs using PIDE-extracted free variables for induction.
   *  Also runs sledgehammer in parallel since we're at the command level where it works correctly. */
  private def trySimpleMethods(
    view: View, command: Command, analysis: GoalExtractor.GoalAnalysis
  ): Option[String] = {
    val candidates = simpleMethods ++ inductionCandidates(analysis.freeVars)
    chat(s"## Phase 0: Simple Methods\nFree variables from PIDE: ${analysis.freeVars.mkString(", ")}")
    ...
    if (IQAvailable.isAvailable && CopilotOptions.getUseSledgehammer) {
      ...
      GUI_Thread.later {
        IQIntegration.runSledgehammerAsync(view, command, timeout, {
          case Right(results) => sledgeResults = results; sledgeLatch.countDown()
    ...
    val sledgeTimeout = CopilotOptions.getSledgehammerTimeout
    ...
    if (sledgeResults.nonEmpty) {
      ...
      val sledgeResult = boundary {
        for (result <- sledgeResults) {
          ...
          val latch = new CountDownLatch(1)
          @volatile var verifyResult: Option[IQIntegration.VerificationResult] = None
          GUI_Thread.later {
            IQIntegration.verifyProofAsync(view, command, method, stepTimeoutMs, { r =>
              verifyResult = Some(r)
              latch.countDown()
    ...
    chat("No simple method or sledgehammer worked, proceeding to sketch-and-fill...")
    None
  }
  ...
  private def sketchPhase(
    view: View, command: Command, commandText: String,
    goalState: String, context: String, plan: String, deadline: Long
  ): Option[String] = {
    SketchGenerator.generateSkeleton(view, command, commandText, goalState, context, plan, deadline)
  }
  ...
  /**
   * Fill phase: iteratively replace sorries with verified proof steps.
   * Tries simple methods first, then LLM, diagnosis-driven retry, refinement, and lemma speculation.
   * Each filled sorry is verified by I/Q before proceeding to the next.
   */
  private def fillPhase(
    view: View, command: Command, commandText: String,
    initialSketch: String, context: String, plan: String, deadline: Long
  ): String = {
    var proof = initialSketch
    Output.writeln(s"[Copilot] fillPhase initial sketch (${proof.length} chars):\n---\n$proof\n---")
    var step = 0
    var refinementCount = 0  // Track total refinements to prevent runaway

    while (countSorries(proof) > 0 && step < maxFillAttempts && !CopilotDockable.isCancelled && System.currentTimeMillis() < deadline) {
      val remaining = countSorries(proof)
      step += 1
      CopilotDockable.setStatus(s"Proving... step $step ($remaining sorries remaining)")

      chat(s"## Fill step $step ($remaining remaining)\n```isabelle\n${markSorry(proof, 0).linesIterator.take(15).mkString("\n")}\n```")
      ...
      val failedErrors: List[(String, String)] = if (filledSimple.isDefined) Nil else {
      ...
      val filled = filledSimple.orElse {
        ...
        fillViaLLM(view, command, commandText, proof, goalForFill, context, plan)
      }.orElse {
        // Phase 2c: Diagnosis-driven retry — ask LLM what went wrong and get specific methods
        if (!CopilotDockable.isCancelled && System.currentTimeMillis() < deadline && failedErrors.nonEmpty) {
          chat("Phase 2c: Diagnosis-driven retry...")
          diagnosisRetry(view, command, commandText, proof, goalForFill, context, plan, failedErrors)
        } else None
      }.orElse {
        // Phase 2d: Refinement — decompose the sorry into a sub-proof with simpler sorries
        if (!CopilotDockable.isCancelled && System.currentTimeMillis() < deadline &&
            refinementCount < maxRefinementDepth * 3) {
          chat("Phase 2d: Refining sorry into sub-proof...")
          refineSorry(view, command, commandText, proof, goalForFill, context, plan).map { newProof =>
            refinementCount += 1
            newProof
          }
        } else None
      ...
      val finalFilled = filled.orElse {
        ...
        if (!CopilotDockable.isCancelled && System.currentTimeMillis() < deadline &&
          ...
          speculateLemma(view, command, commandText, proof, goalForFill, context, plan, failedErrors)
        } else None
      }
      ...
      finalFilled match {
        ...
        case None =>
          ...
          proof = replaceSorry(proof, 0, "sorry (* unfilled *)").getOrElse(proof)
      }
    }
    proof
  }
  ...
  private def diagnosisRetry(
    ...
    try {
      ...
      val response = invokeLLMWithTimeout(PromptLoader.load("prove_diagnose_retry.md", subs))
      val suggestions = SuggestAction.parseLLMSuggestions(response)
      chat(s"Diagnosis suggests ${suggestions.length} methods: ${suggestions.map(s => s"`$s`").mkString(", ")}")
  ...
  private def refineSorry(
    ...
    if (all.isEmpty) { chat("No refinement candidates generated"); None }
      ...
      chat(s"Generated ${all.length} refinement candidates, checking via I/Q...")

      import scala.util.boundary, boundary.break
      boundary {
        for (refinement <- all) {
          if (CopilotDockable.isCancelled) break(None)
          chat(s"Refinement candidate:\n```isabelle\n${refinement.linesIterator.take(8).mkString("\n")}\n```")
          ...
          replaceSorry(proof, 0, refinement) match {
            ...
            case Some(newProof) =>
              ...
              if (newSorries <= oldSorries) {
                chat(s"  Refinement didn't add new sorries ($newSorries ≤ $oldSorries), skipping")
              } else {
                executeStep(view, command, newProof) match {
                  case Right(_) =>
                    chat(s"  [ok] Refinement verified ($newSorries sorries, was $oldSorries)")
                    break(Some(newProof))
                  case Left(err) =>
   ...
   *  recursively using the same proof machinery, insert it before the
   *  main lemma in the buffer, and retry the sorry.
   *  This handles cases like missing left-distributivity lemmas. */
  private def speculateLemma(
    ...
    try {
      ...
      val failedMethodNames = if (failedErrors.nonEmpty) failedErrors.map(_._1)
      ...
      val response = invokeLLMWithTimeout(PromptLoader.load("prove_speculate_lemma.md", subs.toMap))
      ...
      val insertedLength = insertLemmaBeforeCommand(view, command, lemmaWithSorry)
      if (insertedLength <= 0) {
      ...
      // Wait for PIDE to process
      Thread.sleep(CopilotConstants.PIDE_PROCESSING_DELAY)
      ...
      val lemmaProof = proveLemmaFull(view, lemmaCmd.get, lemmaStatement.trim, lemmaGoalState, context)
      ...
      replaceSorryInBuffer(view, lemmaWithSorry, lemmaStatement.trim + "\n" + proofText)

      // Wait for PIDE to re-process
      Thread.sleep(CopilotConstants.PIDE_PROCESSING_DELAY)

      // Verify no errors
      val hasErrors = checkForErrorsAtOffset(view, insertedLength + proofText.length)
      if (hasErrors) {
      ...
      val result = boundary {
        for (method <- methodsToTry) {
          ...
          replaceSorry(proof, 0, method).foreach { newProof =>
            executeStep(view, command, newProof) match {
              case Right(_) =>
                chat(s"  [ok] Sorry filled with `$method` using speculated lemma!")
                break(Some(newProof))
              case Left(err) =>
                chat(s"  [FAIL] `$method`: $err")
            }
      ...
      if (result.isEmpty) {
        chat("  Speculated lemma didn't help close the sorry, undoing insertion...")
        undoLemmaInsertion(view)
      }
      result
    } catch {
      case ex: Exception =>
        Output.writeln(s"[Copilot] Lemma speculation failed: ${ex.getMessage}")
        None
    }
  }

  /** Prove a speculated lemma using the full proof pipeline (Phase 0 → 1 → 2).
   *  This is a simplified version of `run` that operates on the speculated lemma
   *  instead of the main goal. Does NOT call Phase 2e (no recursive speculation).
   *  Returns Some(proofText) or None. */
  private def proveLemmaFull(
    view: View, lemmaCmd: Command, lemmaText: String, goalState: String, context: String
  ): Option[String] = {
    val deadline = System.currentTimeMillis() + CopilotConstants.PROVE_LEMMA_SUB_DEADLINE
    ...
    val goalAnalysis = lemmaOffset.flatMap(off => analyzeGoalAtOffset(view, off))
    ...
    val effectiveVars = if (freeVars.nonEmpty) freeVars
    ...
    chat(s"  Phase 0: Trying ${candidates.length} methods on speculated lemma...")
    ...
    if (IQAvailable.isAvailable && !CopilotDockable.isCancelled) {
      chat(s"  Running sledgehammer on speculated lemma...")
      val timeout = CopilotOptions.getSledgehammerTimeout
      val sledgeLatch = new CountDownLatch(1)
      @volatile var sledgeResults: List[IQIntegration.SledgehammerResult] = Nil
      GUI_Thread.later {
        IQIntegration.runSledgehammerAsync(view, lemmaCmd, timeout, {
          case Right(results) => sledgeResults = results; sledgeLatch.countDown()
          case Left(_) => sledgeLatch.countDown()
        })
      }
    ...
    if (CopilotDockable.isCancelled || System.currentTimeMillis() > deadline) return None

    // Phase 1: Skeleton generation for the speculated lemma
    chat(s"  Phase 1: Generating skeleton for speculated lemma...")
    val skeleton = sketchPhase(view, lemmaCmd, lemmaText, goalState, context, "", deadline)
    if (skeleton.isEmpty || CopilotDockable.isCancelled) return None
    ...
    val filledProof = fillPhaseLimited(view, lemmaCmd, lemmaText, skeleton.get, context, deadline)
  ...
  /** 
   * Limited fill phase for speculated lemmas — runs 2a/2b/2c/2d but NOT 2e
   *  (no recursive lemma speculation to prevent infinite recursion). */
  private def fillPhaseLimited(
    view: View, command: Command, commandText: String,
    initialSketch: String, context: String, deadline: Long
  ): String = {
    var proof = initialSketch
    var step = 0
    val maxSteps = CopilotConstants.PROVE_LEMMA_MAX_STEPS

    while (countSorries(proof) > 0 && step < maxSteps && !CopilotDockable.isCancelled && System.currentTimeMillis() < deadline) {
      val remaining = countSorries(proof)
      step += 1
      ...
      val filled = trySimpleReplacements(view, command, proof).orElse {
        // 2b: LLM
        fillViaLLM(view, command, commandText, proof, goalForFill, context, "")
      }.orElse {
        // 2c: Diagnosis retry
        if (!CopilotDockable.isCancelled && System.currentTimeMillis() < deadline)
          diagnosisRetry(view, command, commandText, proof, goalForFill, context, "")
        else None
      }.orElse {
        // 2d: Refinement
        if (!CopilotDockable.isCancelled && System.currentTimeMillis() < deadline)
          refineSorry(view, command, commandText, proof, goalForFill, context, "")
        else None
      }
      ...
      filled match {
        ...
        case None => proof = replaceSorry(proof, 0, "sorry (* unfilled *)").getOrElse(proof)
      }
    }
    proof
  }
  ...
  private def analyzeGoalAtOffset(view: View, offset: Int): Option[GoalExtractor.GoalAnalysis] =
  ...
  /** Replace text in the buffer (used to swap sorry for actual proof in speculated lemma). */
  private def replaceSorryInBuffer(view: View, oldText: String, newText: String): Unit = {
  ...
  /** 
   * Insert lemma text before the current command in the buffer.
   *  Finds the start of the current top-level command (lemma/theorem) using PIDE
   *  command structure, and inserts the new lemma before it.
   *  Must dispatch buffer edits to the GUI thread.
   *  Returns the length of inserted text (>0 on success, 0 on failure). */
  private def insertLemmaBeforeCommand(view: View, command: Command, lemmaCode: String): Int = {
    val latch = new CountDownLatch(1)
    ...
    GUI_Thread.later {
      try {
        ...
        Document_Model.get_model(buffer).foreach { model =>
          ...
          val targetCmd = node.command_iterator(Text.Range(caretPos, caretPos + 1))
            .toList.headOption
          ...
          val insertOffset = targetCmd match {
            case Some((cmd, cmdOff)) =>
              ...
              if (idx >= 0) {
                var i = idx
                while (i >= 0 && !topLevelStarters.contains(allCmds(i)._1.span.name)) i -= 1
    ...
    latch.await(CopilotConstants.BUFFER_OPERATION_TIMEOUT_SEC, TimeUnit.SECONDS)
    insertedLength
  }

  /** 
   * Check if the PIDE document has errors in the region starting at the cursor.
   *  Used to verify speculated lemmas after insertion. */
  private def checkForErrorsAtOffset(view: View, regionLength: Int): Boolean = {
    val latch = new CountDownLatch(1)
    ...
    GUI_Thread.later {
      try {
        val buffer = view.getBuffer
        val caretPos = view.getTextArea.getCaretPosition
        // Check the region before the caret (where the lemma was inserted)
        val checkStart = math.max(0, caretPos - regionLength - 100)
        val checkEnd = caretPos
        Document_Model.get_model(buffer).foreach { model =>
          val snapshot = Document_Model.snapshot(model)
          val range = Text.Range(checkStart, checkEnd)
          val errors = snapshot.cumulate(range, false,
            Markup.Elements(Markup.ERROR_MESSAGE, Markup.ERROR, Markup.BAD), _ => {
              case _ => Some(true)
            })
          hasErrors = errors.exists(_._2)
        }
      } catch { case _: Exception => }
      latch.countDown()
    }
    latch.await(CopilotConstants.GUI_DISPATCH_TIMEOUT_SEC, TimeUnit.SECONDS)
    hasErrors
  }

  /** 
   * Undo the last buffer edit (the lemma insertion).
   *  Uses jEdit's compound edit mechanism — the insertion was wrapped in beginCompoundEdit/endCompoundEdit. */
  ...
  /** 
   * Get the actual subgoal at the first sorry position by executing the proof prefix.
   * This is critical for ensuring the LLM sees the correct goal context when filling sorries.
   */
  private def getSubgoalAtSorry(view: View, command: Command, proof: String): String = {
    splitAtSorry(proof, 0) match {
      case Some((prefix, _)) if prefix.trim.nonEmpty =>
        val stepResult = executeStep(view, command, prefix.trim)
        stepResult match {
          case Right(result) if result.stateText.trim.nonEmpty
              && !result.stateText.contains("PROOF_COMPLETE")
              && !result.stateText.contains("No subgoals") =>
            Output.writeln(s"[Copilot] Got actual subgoal from prefix: ${result.stateText.take(200)}")
            chat(s"Subgoal:\n```\n${result.stateText.linesIterator.take(8).mkString("\n")}\n```")
            result.stateText
          case Right(result) =>
            Output.writeln(s"[Copilot] Prefix returned complete/empty state, falling back")
            initialGoalState(view)
          case Left(err) =>
            Output.writeln(s"[Copilot] Prefix execution failed ($err), falling back")
            initialGoalState(view)
        }
      case _ =>
        Output.writeln("[Copilot] No prefix before sorry, using original goal")
        initialGoalState(view)
    }
  }

  /** 
   * Try to fill the first sorry via LLM candidates.
   * Generates N parallel LLM suggestions and verifies each via I/Q.
   * Returns Some(newProof) with the sorry replaced, or None.
   */
  private def fillViaLLM(
    ...
    if (candidates.isEmpty) {
      ...
      for (fill <- candidates if result.isEmpty && !CopilotDockable.isCancelled) {
  ...
  /** 
   * Collect the actual subgoals at each unfilled sorry position by executing prefixes.
   * Used for diagnosis output when proof completion fails.
   * Note: This re-executes prefixes for each sorry, which can be slow for many sorries.
   */
  private def collectUnfilledSubgoals(view: View, command: Command, proof: String): String = {
  ...
  def run(view: View, command: Command, commandText: String, initialGoalState: String): Unit = {
    val startTime = System.currentTimeMillis()
    val deadline = startTime + CopilotOptions.getProveTimeout
    CopilotDockable.setStatus("Proving...")
    chat(s"**Auto-prove** starting (branches=$numBranches, max fills=$maxFillAttempts, timeout=${CopilotOptions.getProveTimeout / 1000}s)")

    // Get structured goal analysis from PIDE markup
    val goalAnalysis = analyzeGoalAtCursor(view)
    val freeVars = goalAnalysis.map(_.freeVars).getOrElse(Nil)
    if (freeVars.nonEmpty) Output.writeln(s"[Copilot] Free variables from PIDE: ${freeVars.mkString(", ")}")

    // Fetch context: local facts + definitions + relevant theorems for goal terms
    val context = fetchRichContext(view, command, initialGoalState, goalAnalysis)
    if (context.nonEmpty) chat(s"Fetched ${context.linesIterator.size} lines of context")
    ...
    trySimpleMethods(view, command, phase0Analysis) match {
      case Some(proof) =>
        ...
        reportResult(view, Some(proof), s"complete in ${elapsed}s (simple method)", commandText)
        return
      case None =>
    }
    ...
    val skeleton = sketchPhase(view, command, commandText, initialGoalState, context, plan, deadline)
    if (skeleton.isEmpty || CopilotDockable.isCancelled || System.currentTimeMillis() > deadline) {
      val reason = if (System.currentTimeMillis() > deadline) "overall timeout exceeded" else "no valid skeleton generated"
      reportResult(view, None, reason, commandText)
      return
    }

    // Phase 2: Fill sorries (guided by proof plan)
    val finalProof = fillPhase(view, command, commandText, skeleton.get, context, plan, deadline)
    ...
    if (totalRemaining == 0) {
      ...
      reportResult(view, Some(finalProof), s"complete in ${elapsed}s", commandText)
    } else {
      chat(s"## Result\n$totalRemaining sorry${if (totalRemaining != 1) "s" else ""} could not be filled")
      // Diagnose unfilled obligations — collect actual subgoals at each sorry position
      if (!CopilotDockable.isCancelled) {
  ...
  /**
   * Report the final proof result to the user with appropriate badge and insert link.
   * Dispatches to GUI thread for all UI updates.
   */
  private def reportResult(view: View, proof: Option[String], reason: String, commandText: String): Unit = {
    GUI_Thread.later {
      val totalRemaining = proof.map(p => countSorries(p) + countUnfilled(p)).getOrElse(1)
      proof match {
        case Some(code) if totalRemaining == 0 =>
          CopilotDockable.respondInChat(s"[ok] **Proof found** ($reason):",
            Some((code, InsertHelper.createInsertAction(view, code))))
          CopilotDockable.setBadge(VerificationBadge.Verified(None))
        case Some(code) =>
          CopilotDockable.respondInChat(s"**Partial proof** ($reason):",
            Some((code, InsertHelper.createInsertAction(view, code))))
          CopilotDockable.setBadge(VerificationBadge.Failed(reason.take(30)))
        case None =>
          CopilotDockable.respondInChat(s"No proof found: $reason")
          CopilotDockable.setBadge(VerificationBadge.Failed(reason.take(30)))
      }
