...
/**
 * Dynamic right-click context menu for Isabelle/jEdit.
 * Adapts available actions based on cursor context: proof state, selection,
 * error presence, definition type, and I/Q availability.
 */
class CopilotContextMenu extends DynamicContextMenuService {
  override def createMenu(textArea: JEditTextArea, evt: MouseEvent): Array[JMenuItem] = {
    if (evt == null) Array.empty
      ...
      if (mode == null || mode.getName != "isabelle") Array.empty
        ...
        val menu = new JMenu("Isabelle Copilot")
        ...
        addItem(understandMenu, "Explain") { _ =>
          ...
          text match {
            ...
            case None => CopilotDockable.respondInChat("No command at cursor.")
          }
        }

        if (ctx.onError)
          addItem(understandMenu, "Explain Error")(_ => ExplainErrorAction.explainError(view))

        if (ctx.hasTypeInfo)
          addItem(understandMenu, "Show Type")(_ => ShowTypeAction.showType(view))

        addItem(understandMenu, "Summarize Theory")(_ => SummarizeTheoryAction.summarize(view))
        ...
        if (ctx.inProof) {
          ...
          if (ctx.iqAvailable) {
            ...
            addItem(proofMenu, "Auto-Prove")(_ => ChatAction.chat(view, ":prove"))
            addItem(proofMenu, "Run Sledgehammer")(_ => SledgehammerAction.run(view))
            addItem(proofMenu, "Run Nitpick")(_ => NitpickAction.run(view))
            addItem(proofMenu, "Run Quickcheck")(_ => QuickcheckAction.run(view))
            addItem(proofMenu, "Try Methods")(_ => TryMethodsAction.run(view))
            proofMenu.addSeparator()
            addItem(proofMenu, "Trace Simp")(_ => TraceSimplifierAction.trace(view, "simp"))
            addItem(proofMenu, "Trace Auto")(_ => TraceSimplifierAction.trace(view, "auto"))
            addItem(proofMenu, "Print Context")(_ => PrintContextAction.run(view))
          }
          ...
          if (ctx.hasApplyProof) {
            addItem(proofMenu, "Refactor to Isar") { _ =>
              val block = selection.filter(_.trim.nonEmpty)
        ...
        if (ctx.hasCommand || ctx.hasSelection) {
          ...
          val commandForDoc = CommandExtractor.getCommandAtOffset(buffer, offset)
  ...
  private def addItem(menu: JMenu, label: String)(action: Unit => Unit): Unit = {
