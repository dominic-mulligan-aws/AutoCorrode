...
object ContextFetcher {
  ...
  def getContext(view: View, timeoutMs: Long = 3000): Option[String] = {
    if (SwingUtilities.isEventDispatchThread) {
      Output.warning("[Copilot] ContextFetcher.getContext called from GUI thread — returning None to avoid deadlock")
      return None
    }
    ...
    latch.await(CopilotConstants.CONTEXT_FETCH_TIMEOUT, TimeUnit.MILLISECONDS)
  ...
  def fetchDefinitionsForNames(
    ...
    if (javax.swing.SwingUtilities.isEventDispatchThread) {
      Output.warning("[Copilot] fetchDefinitionsForNames called from GUI thread — returning None")
      return None
    }
