...
object BedrockClient {
  ...
  private val maxRetries = CopilotConstants.MAX_RETRY_ATTEMPTS
  private val baseRetryDelayMs = CopilotConstants.RETRY_BASE_DELAY_MS
      ...
      if (now < circuitOpenUntilMs) {
        ...
        circuitBreakerLock.synchronized {
          if (now >= circuitOpenUntilMs && consecutiveFailures >= circuitBreakerThreshold) {
            Output.writeln("[Copilot] Circuit breaker: cooldown elapsed, allowing probe request")
  ...
  private def recordSuccess(): Unit = circuitBreakerLock.synchronized {
    if (consecutiveFailures > 0) {
      Output.writeln(s"[Copilot] Circuit breaker: reset after success (was $consecutiveFailures failures)")
  ...
  private def recordFailure(): Unit = circuitBreakerLock.synchronized {
    ...
    if (consecutiveFailures >= circuitBreakerThreshold) {
      ...
      Output.writeln(s"[Copilot] Circuit breaker OPEN: $consecutiveFailures consecutive failures, cooldown ${circuitBreakerCooldownMs / 1000}s")
    }
  }
  ...
  private def getClient: BedrockRuntimeClient = {
    val region = CopilotOptions.getRegion
    cachedClient match {
    ...
    ErrorHandler.logOperation("invoke") {
      ...
      LLMResponseCache.get(prompt) match {
        case Some(cachedResponse) =>
          Output.writeln(s"[Copilot] Using cached response (${cachedResponse.length} chars)")
          cachedResponse
        case None =>
      ...
      try {
        ...
        case ex: Exception =>
          ...
          if (attempt < maxAttempts) {
            ...
            Output.writeln(s"[Copilot] Attempt $attempt failed, retrying in ${delay}ms: ${ErrorHandler.makeUserFriendly(ex.getMessage, "request")}")
            Thread.sleep(delay)
  ...
  private def invokeChatInternal(systemPrompt: String, messages: List[(String, String)]): String = {
    ...
    val temperature = CopilotOptions.getTemperature
    val maxTokens = CopilotOptions.getMaxTokens

    val fullSystemPrompt = List(PromptLoader.getSystemPrompt, systemPrompt).filter(_.nonEmpty).mkString("\n\n")

    Output.writeln(s"[Copilot] invokeChat - Model: $modelId, Messages: ${messages.length}")
    ...
    val maxChars = CopilotConstants.MAX_CHAT_CONTEXT_CHARS
    ...
    val truncated = {
      ...
      if (available <= 0) {
        ...
        Output.warning(s"[Copilot] System prompt ($systemCost chars) exceeds context budget ($maxChars chars)")
        List.empty
      } else {
    ...
    if (truncated.length < messages.length)
      Output.writeln(s"[Copilot] invokeChat - Truncated ${messages.length - truncated.length} old messages")
  ...
  private def invokeChatWithTools(
    ...
    val maxIter = CopilotOptions.getMaxToolIterations
    ...
    while (iteration < maxIter) {
      iteration += 1
      if (CopilotDockable.isCancelled) throw new RuntimeException("Operation cancelled")
      ...
      if (toolUses.isEmpty || stopReason != "tool_use") {
        ...
        val resultBlocks = toolUses.map { tu =>
          Output.writeln(s"[Copilot] Tool use ($iteration/$maxIter): ${tu.name}")
          CopilotDockable.setStatus(s"[tool] ${tu.name} ($iteration/$maxIter)...")
          val result = CopilotTools.executeTool(tu.name, tu.input, view)
          (tu.id, result)
        }
    ...
    Output.writeln(s"[Copilot] Tool loop completed in $iteration iterations, response: ${finalText.length} chars")
    finalText
  }
  ...
  private def invokeInternal(prompt: String): String = {
    ...
    Output.writeln(s"[Copilot] Region: $region")
    Output.writeln(s"[Copilot] Model: $modelId")
    Output.writeln(s"[Copilot] Temperature: $temperature, Max tokens: $maxTokens")
    Output.writeln(s"[Copilot] Prompt length: ${fullPrompt.length} chars")
    ...
    Output.writeln(s"[Copilot] Parsed response length: ${parsed.length} chars")
    parsed
  }
