 ...
 * logging for Isabelle Copilot.
 *
 * Provides:
 * - User-friendly error message translation
 * - Input validation and sanitization
 * - Operation timing/logging
 * - Standardized Try-based error handling with optional post-hoc timeout check
...
object ErrorHandler {
  ...
  /**
   * Validate user input: reject null/empty, but do NOT truncate.
   * Use `sanitize` if truncation is desired.
   */
  def validateInput(input: String): Try[String] = {
    if (input == null) Failure(new IllegalArgumentException("Input cannot be null"))
    else if (input.trim.isEmpty) Failure(new IllegalArgumentException("Input cannot be empty"))
    else scala.util.Success(input.trim)
  }

  /** Truncate a string to `maxLength`, appending "..." if truncated. */
  def sanitize(input: String, maxLength: Int = CopilotConstants.MAX_RESPONSE_LENGTH): String =
    if (input == null) ""
  ...
  def makeUserFriendly(message: String, operation: String): String = {
    if (message == null || message.trim.isEmpty) s"Unknown error in $operation"
      ...
      else if (msg.contains("credentials") || msg.contains("unauthorized") || msg.contains("access denied"))
      ...
      else if (msg.contains("quota") || msg.contains("throttle") || msg.contains("rate limit") || msg.contains("too many requests"))
  ...
  def truncateError(error: String): String = {
    ...
    else if (error.length > CopilotConstants.MAX_ERROR_MESSAGE_LENGTH)
      error.take(CopilotConstants.MAX_ERROR_MESSAGE_LENGTH) + "..."
    else error
  }

  // --- Operation logging ---

  /** Safe wrapper for Output.writeln — no-op if Isabelle runtime isn't available. */
  private def safeWriteln(msg: String): Unit =
    try { Output.writeln(msg) } catch { case _: Throwable => System.err.println(msg) }

  /** Safe wrapper for Output.error_message — no-op if Isabelle runtime isn't available. */
  private def safeErrorMessage(msg: String): Unit =
    try { Output.error_message(msg) } catch { case _: Throwable => System.err.println(s"ERROR: $msg") }

  /** Log operation with timing information. Re-throws with user-friendly message on failure. */
  def logOperation[T](operation: String)(block: => T): T = {
    val startTime = System.currentTimeMillis()
    try {
      val result = block
      val elapsed = System.currentTimeMillis() - startTime
      safeWriteln(s"[Copilot] $operation completed in ${elapsed}ms")
      result
    } catch {
      case ex: Exception =>
        val elapsed = System.currentTimeMillis() - startTime
        val userMessage = makeUserFriendly(ex.getMessage, operation)
        safeErrorMessage(s"[Copilot] $operation failed after ${elapsed}ms: $userMessage")
  ...
  /**
   * Register a resource for automatic cleanup on plugin stop.
   * Oldest entries are closed and dropped when the registry exceeds capacity.
   */
   ...
   */
  def withErrorHandling[T](operation: String, elapsedLimitMs: Option[Long] = None)(block: => T): Try[T] = {
    val startTime = System.currentTimeMillis()
    Try {
      val result = block
      elapsedLimitMs.foreach { limit =>
        val elapsed = System.currentTimeMillis() - startTime
        if (elapsed > limit)
          throw new RuntimeException(s"$operation exceeded elapsed limit after ${elapsed}ms (limit: ${limit}ms)")
      }
      result
    }.recoverWith {
      case ex: java.util.concurrent.TimeoutException =>
        val msg = s"$operation timed out. Try reducing complexity or check connection."
        safeErrorMessage(s"Timeout in $operation: $msg")
        Failure(new RuntimeException(msg, ex))
      case ex: java.net.ConnectException =>
        val msg = s"Cannot connect to service. Check internet connection and credentials."
  ...
  /**
   * Safe resource management with automatic cleanup after use.
   * The resource is closed immediately when the block completes (or throws).
   */
  def withManagedResource[R <: AutoCloseable, T](resource: => R)(block: R => T): Try[T] = {
    ...
    res.foreach { r => try { r.close() } catch { case _: Throwable => } }
    result
  }
}
