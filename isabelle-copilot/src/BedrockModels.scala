...
object BedrockModels {
  private val cacheFile: Path =
    Paths.get(System.getProperty("user.home"), ".isabelle", "copilot_models.txt")
  ...
  def refreshModels(region: String): Array[String] = {
    Output.writeln(s"[Copilot] Refreshing models from region: $region")
    ...
    try {
      ...
      val foundationModels = foundationResponse.modelSummaries().asScala
        .map(_.modelId())
        .toList
      Output.writeln(s"[Copilot] Found ${foundationModels.size} foundation models")
      ...
      val profileModels = profileResponse.inferenceProfileSummaries().asScala
        ...
        .toList
      Output.writeln(s"[Copilot] Found ${profileModels.size} inference profiles")

      // Combine, deduplicate, and sort all text models
      val allModels = (foundationModels ++ profileModels).distinct.sorted.toArray
      Output.writeln(s"[Copilot] Total text models: ${allModels.length}")

      // Write cache
      Files.createDirectories(cacheFile.getParent)
      Files.write(cacheFile, allModels.toSeq.asJava)
      Output.writeln(s"[Copilot] Models cached to: $cacheFile")

      allModels
    } catch {
      case ex: Exception =>
        Output.writeln(s"[Copilot] Error refreshing models: ${ex.getMessage}")
        throw ex
    } finally {
      client.close()
    }
