...
object PromptLoader {
  ...
  /** Resolve ISABELLE_COPILOT_HOME, returning None with a warning if unset. */
  private def copilotHome: Option[String] = {
    val home = Isabelle_System.getenv("ISABELLE_COPILOT_HOME")
    if (home.isEmpty) {
      Output.warning("[Copilot] ISABELLE_COPILOT_HOME not set. Install the plugin via 'make install' or set the variable manually.")
      None
    } else Some(home)
  }

  def load(name: String, substitutions: Map[String, String]): String = {
    val home = copilotHome.getOrElse(
      error("ISABELLE_COPILOT_HOME not set — install the plugin via 'make install' or add init_component to settings"))
