...
object SummarizeTheoryAction {
  ...
  private def estimateContextWindow: Int = {
    val modelId = CopilotOptions.getModelId.toLowerCase
  ...
  private def doSummarize(view: View, buffer: org.gjt.sp.jedit.Buffer, source: String): Unit = {
    ...
    val promptOpt = try {
      ...
      case ex: Exception =>
        GUI.error_dialog(view, "Isabelle Copilot", "Failed to load prompt: " + ex.getMessage)
        None
    }

    promptOpt.foreach { prompt =>
      ActionHelper.runAndRespond("copilot-summarize", "Summarizing theory...") {
