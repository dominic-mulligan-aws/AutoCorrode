...
object SuggestAction {
  ...
  private def suggestInternal(view: View, buffer: JEditBuffer, offset: Int): Unit = {
    ErrorHandler.withErrorHandling("suggest operation") {
      ...
      (commandText, goalState) match {
        case (None, _) =>
          GUI.warning_dialog(view, "Isabelle Copilot", "No command at cursor position")
        case (_, None) =>
          GUI.warning_dialog(view, "Isabelle Copilot", "No goal state available")
        case (Some(cmd), Some(goal)) =>
          val commandObj = IQIntegration.getCommandAtOffset(buffer, offset)
          val canVerify = IQAvailable.isAvailable && CopilotOptions.getVerifySuggestions && commandObj.isDefined
          val useSledgehammer = CopilotOptions.getUseSledgehammer && IQAvailable.isAvailable && commandObj.isDefined

          CopilotDockable.setStatus("Generating suggestions...")

          Isabelle_Thread.fork(name = "copilot-suggest") {
            ErrorHandler.withErrorHandling("suggestion generation") {
              Output.writeln(s"[Copilot] Starting suggestion collection...")
              val candidates = collectCandidates(view, cmd, goal, commandObj, useSledgehammer)
              Output.writeln(s"[Copilot] Collected ${candidates.length} candidates")
              
              val verified = if (canVerify) {
                Output.writeln(s"[Copilot] Starting verification...")
                verifyCandidates(view, commandObj.get, candidates)
              } else candidates
              
              Output.writeln(s"[Copilot] After verification: ${verified.length} candidates")
              val ranked = rankCandidates(verified)
              Output.writeln(s"[Copilot] Displaying ${ranked.length} ranked candidates")
  ...
  private def collectCandidates(
    ...
    Output.writeln(s"[Copilot] Starting $taskCount tasks, sledgehammer=$useSledgehammer")
    ...
    // Wait for completion
    val waited = latch.await(CopilotConstants.SUGGESTION_COLLECTION_TIMEOUT, TimeUnit.MILLISECONDS)
    Output.writeln(s"[Copilot] Latch done, waited=$waited, results count: ${results.size}")
  ...
  private def prepareContextInfo(view: View, commandObj: Option[Command], goalState: String, useIQ: Boolean): String = {
    if (!useIQ) ""
      ...
      factsLatch.await(CopilotConstants.CONTEXT_FETCH_TIMEOUT + CopilotConstants.TIMEOUT_BUFFER_MS, java.util.concurrent.TimeUnit.MILLISECONDS)
      theoremsLatch.await(CopilotConstants.CONTEXT_FETCH_TIMEOUT + CopilotConstants.TIMEOUT_BUFFER_MS, java.util.concurrent.TimeUnit.MILLISECONDS)
  ...
  private def startLLMTask(
    ...
    Isabelle_Thread.fork(name = "suggest-llm") {
      try {
        ...
        Output.writeln(s"[Copilot] Suggest - Goal state:\n$goalState")
        Output.writeln(s"[Copilot] Suggest - Context:\n$contextInfo")
        
        val prompt = PromptLoader.load("suggest_proof_step.md", subs.toMap)
        Output.writeln(s"[Copilot] Suggest - Prompt length: ${prompt.length}")
        
        val response = BedrockClient.invokeInContext(prompt)
        Output.writeln(s"[Copilot] Suggest - LLM response:\n$response")
        
        val suggestions = parseLLMSuggestions(response)
        Output.writeln(s"[Copilot] Suggest - Parsed ${suggestions.length} suggestions: $suggestions")
        
        suggestions.foreach(s => results.add(Candidate(s, LLM)))
        Output.writeln(s"[Copilot] Added ${suggestions.length} LLM candidates, total now: ${results.size}")
      } catch { 
        case ex: Exception => 
          Output.writeln(s"[Copilot] Suggest - LLM error: ${ex.getMessage}")
      }
      finally { 
        latch.countDown() 
        Output.writeln(s"[Copilot] LLM task done, latch count: ${latch.getCount}")
      }
    }
  }
  ...
  private def startSledgehammerTask(
    ...
    GUI_Thread.later {
      IQIntegration.runSledgehammerAsync(
        view, command, CopilotOptions.getSledgehammerTimeout,
        {
    ...
    Isabelle_Thread.fork(name = "sledge-guard") {
      ...
      Thread.sleep(CopilotOptions.getSledgehammerTimeout + CopilotConstants.SLEDGEHAMMER_GUARD_TIMEOUT)
      if (sledgeCompleted.compareAndSet(false, true)) {
        latch.countDown()
      }
    }
  ...
  private def getContextFacts(view: View, command: Command): String = {
    ErrorHandler.withErrorHandling("context facts retrieval", Some(CopilotConstants.CONTEXT_FETCH_TIMEOUT)) {
      ...
      latch.await(CopilotConstants.CONTEXT_FETCH_TIMEOUT + CopilotConstants.TIMEOUT_BUFFER_MS, TimeUnit.MILLISECONDS)
      contextResult
    }.getOrElse("")
  }
  ...
  private def findRelevantTheorems(view: View, command: Command, goalState: String): String = {
    ...
    val constantsFromMarkup = GoalExtractor.analyzeGoal(buffer, offset)
      .map(_.constants.take(CopilotConstants.MAX_CONSTANTS_FOR_FIND_THEOREMS))
      .getOrElse(Nil)
    
    // Fallback to regex extraction if PIDE markup didn't provide constants
    val constants = if (constantsFromMarkup.nonEmpty) {
      Output.writeln(s"[Copilot] Using constants from PIDE markup: ${constantsFromMarkup.mkString(", ")}")
      constantsFromMarkup
    } else {
      ...
      val fromText = goalState.linesIterator
        ...
        .toSet
        .take(CopilotConstants.MAX_CONSTANTS_FOR_FIND_THEOREMS)
        .toList
      Output.writeln(s"[Copilot] Using constants from text heuristic: ${fromText.mkString(", ")}")
      fromText
    }
    ...
    Output.writeln(s"[Copilot] find_theorems criteria: ${criteria.mkString(", ")}")
    
    if (criteria.isEmpty) ""
    else {
      val limit = CopilotOptions.getFindTheoremsLimit
      val timeout = CopilotOptions.getFindTheoremsTimeout
      ...
      GUI_Thread.later {
        IQIntegration.runFindTheoremsAsync(view, command, pattern, limit, timeout, {
          case Right(results) =>
            Output.writeln(s"[Copilot] find_theorems found ${results.length} results")
            theorems = results.take(limit)
            latch.countDown()
          case Left(err) =>
            Output.writeln(s"[Copilot] find_theorems error: $err")
            latch.countDown()
        })
      }
  ...
  def parseLLMSuggestions(response: String): List[String] = {
    ...
    (primary ++ fallback ++ fromBlocks)
      .distinct
      .take(CopilotOptions.getMaxVerifyCandidates)
  }
  ...
  private def verifyCandidates(view: View, command: Command, candidates: List[Candidate]): List[Candidate] = {
    ...
    val maxCandidates = CopilotOptions.getMaxVerifyCandidates
  ...
  private def displayResults(view: View, candidates: List[Candidate]): Unit = {
    if (candidates.isEmpty) {
      CopilotDockable.respondInChat("No suggestions found.")
    } else {
