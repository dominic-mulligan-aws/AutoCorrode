...
object NitpickAction {
  def run(view: View): Unit = {
    ...
    if (commandOpt.isEmpty) {
      ...
      CopilotDockable.setStatus("Running nitpick...")
      val timeout = CopilotOptions.getNitpickTimeout
  ...
  private def displayResult(view: View, result: Either[String, String], goal: String): Unit = {
    result match {
      ...
      case Right(output) if output.toLowerCase.contains("no counterexample") =>
        CopilotDockable.respondInChat("Nitpick: No counterexample found.")
      case Right(_) =>
        CopilotDockable.respondInChat("Nitpick executed. Check the Output panel for results.")
      case Left(error) =>
        CopilotDockable.respondInChat(s"Nitpick error: $error")
    }
  }
}
