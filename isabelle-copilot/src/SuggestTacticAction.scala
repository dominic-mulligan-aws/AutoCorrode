...
object SuggestTacticAction {
  ...
  def suggest(view: View, proofPattern: String): Unit = {
    val hasEisbach = CopilotSupport.hasEisbach(view.getBuffer)
    ...
    ActionHelper.runAsync("copilot-suggest-tactic", "Generating Eisbach method...") {
  ...
  private def showMethodResult(view: View, methodLine: String, hasEisbach: Boolean, badge: VerificationBadge.BadgeType): Unit = {
    ...
    if (!hasEisbach) {
      val importId = CopilotDockable.registerAction(() =>
        view.getBuffer.insert(view.getTextArea.getCaretPosition, CopilotSupport.importSuggestion))
      sb.append(s"Warning: Eisbach not imported. Add: `${CopilotSupport.importSuggestion}` {{INSERT:$importId}}\n\n")
    }
    ...
    val methodId = CopilotDockable.registerAction(() =>
