...
/**
 * Singleton companion for the Copilot dockable panel.
 ...
 */
object CopilotDockable {
  @volatile private var instance: Option[CopilotDockable] = None
  ...
  private val maxInsertActions = CopilotConstants.MAX_INSERT_ACTIONS
  @volatile private var _cancelled = false
  @volatile private var _busy = false
  @volatile private var _busyStartTime = 0L

  def setInstance(d: CopilotDockable): Unit = synchronized { instance = Some(d) }
  ...
  def cancel(): Unit = {
    _cancelled = true
    _busy = false
    setStatus(CopilotConstants.STATUS_CANCELLED)
  }
  ...
  def setStatus(status: String): Unit = {
    val wasBusy = _busy
    _busy = status != CopilotConstants.STATUS_READY && status != CopilotConstants.STATUS_CANCELLED && !status.startsWith("Error")
 ...
 */
class CopilotDockable(view: View, position: String) extends Dockable(view, position) {

  CopilotDockable.setInstance(this)
      ...
      def hyperlinkUpdate(e: HyperlinkEvent): Unit = {
        if (e.getEventType == HyperlinkEvent.EventType.ACTIVATED) {
          ...
          if (desc != null && desc.startsWith("action:insert:")) {
            ...
            val action = CopilotDockable.insertActions.get(id)
  ...
  private def createControlElements(): (JLabel, JButton, JButton) = {
    val status = new JLabel(CopilotConstants.STATUS_READY)
  ...
  private def createInputPanel(): (JTextArea, JButton, JPanel) = {
    val input = new JTextArea(CopilotConstants.CHAT_INPUT_ROWS, CopilotConstants.CHAT_INPUT_COLUMNS)
  ...
  private def setupButtonHandlers(): Unit = {
    ...
    cancelButton.addActionListener(_ => CopilotDockable.cancel())
  }
  ...
  private def setupChatInputHandlers(): Unit = {
    ...
    actionMap.put("cancel-or-clear", new javax.swing.AbstractAction() {
      def actionPerformed(e: java.awt.event.ActionEvent): Unit = {
        if (CopilotDockable._busy) CopilotDockable.cancel()
        else clearChat()
      }
    })
  }
  ...
  private def setupAccessibilityHandlers(): Unit = {
    ...
    chatInput.getAccessibleContext.setAccessibleDescription("Type your message to the Isabelle Copilot. Enter sends, Shift+Enter for newline.")
    ...
    statusLabel.getAccessibleContext.setAccessibleDescription("Current status of the Isabelle Copilot")
  ...
  private def setupStatusHandlers(): Unit = {
    iqStatusLabel.addMouseListener(new java.awt.event.MouseAdapter() {
      override def mouseClicked(e: java.awt.event.MouseEvent): Unit = showCopilotHelp()
    })

    PIDE.session.commands_changed += Session.Consumer("copilot-status") { _ =>
      GUI_Thread.later { updateCopilotStatus() }
    }
  }

  private def clearChat(): Unit = {
    ChatAction.clearHistory()
    CopilotDockable.clearInsertActions()
    htmlPane.setText("")
  ...
  private val maxInputLength = CopilotConstants.MAX_CHAT_CONTEXT_CHARS / 2  // 50K chars max per message
    ...
    if (message.nonEmpty) {
      if (message.length > maxInputLength) {
        javax.swing.JOptionPane.showMessageDialog(this,
          ...
          "Isabelle Copilot", javax.swing.JOptionPane.WARNING_MESSAGE)
      } else {
        chatInput.setText("")
        CopilotDockable.resetCancel()
  ...
  def displayConversation(history: List[ChatAction.Message]): Unit = displayLock.synchronized {
    ...
    if (canIncrement) {
      ...
      val newHtml = newMessages.map { msg =>
        ...
        else createCopilotMessageHtml(msg.content, ChatAction.formatTime(msg.timestamp), msg.rawHtml)
      }.mkString
  ...
  def updateStatus(status: String): Unit = {
    val displayStatus = if (CopilotDockable._busy && CopilotDockable._busyStartTime > 0) {
      val elapsed = (System.currentTimeMillis() - CopilotDockable._busyStartTime) / 1000
      if (elapsed >= 2) s"$status (${elapsed}s)" else status
    } else status
    statusLabel.setText(displayStatus)
    cancelButton.setVisible(CopilotDockable._busy)
  }

  def updateCopilotStatus(): Unit = {
    val buffer = view.getBuffer
    val status = CopilotSupport.getStatus(buffer)
    iqStatusLabel.setBackground(CopilotSupport.statusColor(status))
    iqStatusLabel.setForeground(Color.WHITE)
    iqStatusLabel.setText(CopilotSupport.statusText(status))
  }

  def updateIQStatus(): Unit = updateCopilotStatus()

  private def showCopilotHelp(): Unit = {
    val buffer = view.getBuffer
    JOptionPane.showMessageDialog(this, CopilotSupport.helpText(buffer),
      "Isabelle Copilot Help", JOptionPane.INFORMATION_MESSAGE)
  }

  def updateModelLabel(): Unit = {
    val modelId = CopilotOptions.getModelId
  ...
  private def createCopilotMessageHtml(content: String, timestamp: String, rawHtml: Boolean = false): String = {
    ...
    val body = if (rawHtml) content else {
      val registerAction = (code: String) => {
        val v = view
        CopilotDockable.registerAction(InsertHelper.createInsertAction(v, code))
      }
    ...
    s"""<div style='margin:6px 0;padding:8px 10px;background:$bg;border-left:3px solid $border;border-radius:3px;overflow-x:hidden;word-wrap:break-word;'>
       |<div style='font-size:10pt;color:$tsColor;margin-bottom:3px;'><b>Copilot</b> · $timestamp</div>
       |<div>$body</div>
       |</div>""".stripMargin
  }
  ...
  private def createWelcomeHtml(): String = {
    val helpId = CopilotDockable.registerAction(() => ChatAction.chat(view, ":help"))
    ...
    val modelWarning = if (CopilotOptions.getModelId.isEmpty) {
      ...
      s"""<div style='margin-top:6px;padding:6px 8px;background:$eBg;border:1px solid $eBorder;border-radius:3px;font-size:11pt;color:$eText;'>
         ...
         |<b>Plugins → Plugin Options → Isabelle Copilot</b> to set one.
    ...
    s"""<div style='margin:8px 0;padding:10px 12px;background:$wBg;border:1px solid $wBorder;border-radius:4px;'>
       |<div style='font-weight:bold;color:$wTitle;margin-bottom:4px;'>Isabelle Copilot</div>
