...
object ExtractLemmaAction {
  ...
  def extract(view: View, selectedText: String): Unit = {
    ...
    if (context.isEmpty) {
      GUI.warning_dialog(view, "Isabelle Copilot", "Could not determine proof context")
    } else {
      val ctx = context.get
      ...
      CopilotDockable.setStatus("Extracting lemma...")
      CopilotDockable.setBadge(VerificationBadge.Verifying)

      Isabelle_Thread.fork(name = "copilot-extract") {
        try {
          ...
          parseExtractionResponse(response) match {
            ...
            case None =>
              GUI_Thread.later {
                ...
                CopilotDockable.setStatus("Could not parse extraction result")
              }
          }
        } catch {
          case ex: Exception =>
  ...
  private def startVerification(view: View, command: Command, ctx: ExtractionContext, result: ExtractionResult, attempt: Int): Unit = {
    ...
    CopilotDockable.setStatus(s"Verifying extracted lemma (attempt $attempt/$maxRetries)...")

    IQIntegration.verifyProofAsync(view, command, result.extractedLemma, timeout, {
      case IQIntegration.ProofSuccess(_, _) =>
        CopilotDockable.setStatus("Verifying updated proof...")
  ...
  private def retryInBackground(view: View, command: Command, ctx: ExtractionContext, failedResult: ExtractionResult, error: String, attempt: Int, maxRetries: Int): Unit = {
    ...
    Isabelle_Thread.fork(name = "copilot-extract-retry") {
      try {
  ...
  private def showResult(view: View, result: ExtractionResult, badge: VerificationBadge.BadgeType): Unit = {
    ...
    CopilotDockable.respondInChat(s"Extracted lemma '${result.lemmaName}':", Some((code, () => {
