...
object PrintContextAction {
  ...
  def getContextString(view: View): Option[String] = {
    if (javax.swing.SwingUtilities.isEventDispatchThread) {
      Output.warning("[Copilot] PrintContextAction.getContextString called from GUI thread — returning None to avoid deadlock")
      return None
    }
  ...
  def run(view: View): Unit = {
    ...
    if (commandOpt.isEmpty) {
      ...
      CopilotDockable.setStatus("Getting context...")
  ...
  private def displayResult(result: Either[String, String]): Unit = {
    result match {
      case Right(output) if output.trim.nonEmpty =>
        CopilotDockable.respondInChat(s"Context:\n\n```\n${output.trim}\n```")
      case Right(_) =>
        CopilotDockable.respondInChat("No local facts in scope.")
      case Left(error) =>
        CopilotDockable.respondInChat(s"Error: $error")
    }
  }
}
