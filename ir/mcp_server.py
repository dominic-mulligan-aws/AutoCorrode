...
def session_info() -> str:
    ...
    theories = apply_transforms(repl_client.send('Ir.theories ();'))
...
def sledgehammer(repl: str, timeout_secs: int = 15) -> str:
...
def source_map(theory_name: str) -> str:
    return repl_client.send(f'/source-map "{theory_name}"')
...
def init_at_line(id: str, theory_or_file: str, line: int) -> str:
    resolution = repl_client.send(f'/resolve "{theory_or_file}" {line}')
    ...
    if not spec or spec.startswith("ERR") or spec.startswith("No ") or \
       spec.startswith("Cannot") or spec.startswith("Usage"):
        return spec
    return repl_client.send(f"Ir.init {ml_str(id)} [{ml_str(spec)}];")
